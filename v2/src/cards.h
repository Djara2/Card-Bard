...
{
	unsigned short x;
 	// signed short is used so that -1 can be used to indicate that only the backside 
	// (not the alternate answers buffer) should be reviewed when going over incorrect answers.
	// Thus, -1 is used when a user answers incorrectly 
	short y;	        
};
